...
// Chat type definition (local state for now)
type Message = {
    id: string;
    text: string;
    sender: 'me' | 'them';
...
export default function SessionChat() {
    ...
    const [messages, setMessages] = useState<Message[]>([
        {
            id: '1',
            text: 'Hello! Welcome to the tutoring session. 👋',
            sender: 'them',
    ...
    // Stop bouncing after first interaction
                        ...
                        <form onSubmit={handleSend} className="p-4 border-t border-white/10 bg-white/40 dark:bg-black/40 backdrop-blur-md">
                            <div className="flex gap-2">
                                <input
                                    ...
                                    className="flex-1 px-4 py-2 rounded-xl bg-white/50 dark:bg-black/50 border border-white/10 focus:outline-none focus:ring-2 focus:ring-[var(--color-primary)] text-sm text-[var(--color-text-primary)] placeholder-[var(--color-text-secondary)]"
                                />
                                <button
                                    type="submit"
